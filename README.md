...
- **统一自选**：最多 20 只（股票 + 场内 ETF/LOF 混排，不再分池）
...
- 钉选 **1–5 只**，展示与完整看板 **相同的标准诊断卡片**；**自动刷新仍针对全池最多 20 只**；卡片 ↻ **仅刷新当前证券**
- 无边框半透明、始终置顶（可调）、系统托盘
- **🆕 桌面通知预警**：AI 自动关键位计算 + 价格突破弹窗 + 高风险 signal 预警（详见功能 8）
...
| AI 诊断 | OpenAI 兼容接口（如 DeepSeek）；休市时快照不变则沿用缓存结论；**解析失败自动重试**直至成功；**🆕 AI 自动输出支撑/压力位** |
| 桌面通知预警 | **🆕 仅 Electron**：价格突破关键位弹窗、高风险 signal 弹窗、API/行情异常通知；智能降频监控 |
