import type { AppConfig, DiagnosisResult, QuoteSnapshot, SessionLabel } from '../types';
import { formatVolumeForPrompt } from './quoteJsonp';

const SYSTEM_PROMPT = `你是一个专业的 A 股短线量化交易助手。请根据我提供的实时行情快照进行客观的技术面诊断。
...
function buildUserMessage(
  ...
  return `【${snapshot.name} ${snapshot.code} | ${typeLabel(snapshot.instrumentType)}】对齐时刻 ${alignedTime} | ${session}
...
export async function runDiagnosis(
  ...
  const body = {
    ...
    messages: [
      ...
      {
        role: 'user',
        content: buildUserMessage(snapshot, alignedTime, session),
      },
    ],
